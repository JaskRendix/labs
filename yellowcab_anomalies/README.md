...
The anomaly comparison project started with a simple question: what’s changed between 2019 and 2025? But once I dug into the NYC taxi data, it stopped being simple. Patterns weren't just shifting, they were bending, breaking, and reappearing in unexpected places.

I didn't want to just flag weird trips. I wanted to understand them. Speed spikes, fare jumps, tip drop-offs, each one felt like a clue in a bigger story about how the city moves. Adding machine learning gave the whole thing teeth. Isolation Forest didn't just detect outliers, it whispered where the system was fraying.

Mapping it all back to zones made the data feel alive. Suddenly, I wasn't just looking at numbers, I was watching neighborhoods change, one flagged ride at a time.
...
I've got a hunch about where to take this next, but first, I need to see if my old PC can handle the weight. Last time, it choked after half an hour, probably buried under the sheer volume of data. So now I'm thinking: maybe it's not about brute force. Maybe I need a smarter angle, something leaner, more surgical. A way to slice through the noise without frying my machine. If I can pull that off, the next layer of insight might be just one clever pivot away.
